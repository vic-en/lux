    ...
    ui(new Ui::ProfitExplorerPage)
{
    ...
    // staking settings
    QSettings settings;
    nPreferredBlockSize = settings.value("nPreferredBlockSize", (qint64) (2 * COIN)).toLongLong();
...
void ProfitExplorerPage::loadStakeChart(bool firstRun)
{
    ...
    if(!firstRun)
    {
        ...
        int nExpectedTime = staking ? (GetTargetSpacing(nBestHeight) * nNetworkWeight / nWeight) : -0;
    ...
    ui->customPlot->yAxis->setLabel("$LUX Minted");
    ...
    ui->velocityPlot->yAxis->setLabel("$LUX");
